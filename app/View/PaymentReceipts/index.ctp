<div class="paymentReceipts index">
<?php 
	echo "<h2>".__('Payment Receipts')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			...
			$pageHeader.="<th>".$this->Paginator->sort('payment_amount')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('currency_id')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('operator_id')."</th>";
			$pageHeader.="<th>".$this->Paginator->sort('shift_id')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			...
			$excelHeader.="<th>".$this->Paginator->sort('payment_amount')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('currency_id')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('operator_id')."</th>";
			$excelHeader.="<th>".$this->Paginator->sort('shift_id')."</th>";
	...
	foreach ($paymentReceipts as $paymentReceipt){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".h($paymentReceipt['PaymentReceipt']['payment_date'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($paymentReceipt['PaymentReceipt']['payment_amount'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($paymentReceipt['Currency']['abbreviation'], array('controller' => 'currencies', 'action' => 'view', $paymentReceipt['Currency']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($paymentReceipt['Operator']['name'], array('controller' => 'operators', 'action' => 'view', $paymentReceipt['Operator']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($paymentReceipt['Shift']['name'], array('controller' => 'shifts', 'action' => 'view', $paymentReceipt['Shift']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($paymentReceipt['PaymentMode']['name'], array('controller' => 'payment_modes', 'action' => 'view', $paymentReceipt['PaymentMode']['id']))."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $paymentReceipt['PaymentReceipt']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $paymentReceipt['PaymentReceipt']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $paymentReceipt['PaymentReceipt']['id']), array(), __('Are you sure you want to delete # %s?', $paymentReceipt['PaymentReceipt']['id']));
			$pageRow.="</td>";
