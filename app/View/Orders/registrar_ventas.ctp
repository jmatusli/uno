...
/*
  ...
  $('body').on('change','.quantity div input',function(){	
    ...
    $(this).closest('tr').find('td.unabridged_price input').val(unitprice*newQuantity);
...
*/  
  ...
  $('body').on('change','.final div input',function(){
    ...
    if (!admissibleValue){
      ...
      $(this).closest('tr').find('td.unabridged_price input').val(0);  
      ...
      if (fuelshiftid < nightshiftid){
        if (fuelshiftid < afternoonshiftid){
          ...
          if (fuelQuantity>0){
            ...
            afternoonhoserow.find('td.unabridged_price input').val(unitPrice*fuelQuantity);
          }
          else {
            afternoonhoserow.find('td.quantity div input').val(0);
            afternoonhoserow.find('td.price div input').val(0);
            afternoonhoserow.find('td.unabridged_price input').val(0);
          }
        }
        nighthoserow=$("table[fuelshiftid='"+nightshiftid+"']").find('tr#'+hoseid);
        var finalquantitynight=parseFloat(nighthoserow.find('td.final input').val());
        ...
        if (fuelQuantity>0){
          ...
          nighthoserow.find('td.unabridged_price input').val(unitPrice*fuelQuantity);
        }
        else {
          nighthoserow.find('td.quantity div input').val(0);
          nighthoserow.find('td.price div input').val(0);
          nighthoserow.find('td.unabridged_price input').val(0);
        }
      }
    }
    else {
      ...
      $(this).closest('tr').find('td.unabridged_price input').val(unitPrice*fuelQuantity);
  ...
  function calculateTotalFuels(shiftContainer){
    ...
    ?>
        ...
        currentPrice = parseFloat($(this).find('td.unabridged_price input').val());
  ...
  function calculateCalibratedPrice(calibrationTableId){
    ...
    <?php 
      if ($enterpriseId > 0){
        ...
        foreach ($fuels as $fuel){
          echo "fuelPrices[".$fuel['Product']['id']."]=0;\n";
        }
      }
    ?>
  ...
  function calculateTotal(){
    ...
    //$('#totalPriceFuels').val(roundToTwo(netPriceFuels+netPriceCalibrations))
        ...
        echo "<div class='col-sm-12'>";
          ...
          echo "<div class='col-sm-12' style='padding:5px;'>";
            if ($enterpriseId > 0){
              ...
              echo "<h3>Seleccione una gasolinera para ver o grabar datos</h3>";
            }
            foreach ($shifts as $shiftId=>$shiftName){
              $shiftTable="";
                  ...
                  foreach ($islands as $island){
                    ...
                    foreach ($island['Hose'] as $hose){
                      ...
                      */
                        ...
                        $shiftTableRow.="<td class='unabridged_price hidden'>".$this->Form->input('Shift.'.$shiftId.'.Island.'.$island['Island']['id'].'.Hose.'.$hose['id'].'.unabridged_price',['label'=>false,'default'=>0,'value'=>(empty($requestShifts['Shift'][$shiftId]['Island'][$island['Island']['id']]['Hose'][$hose['id']]['price'])?0:(float)$requestShifts['Shift'][$shiftId]['Island'][$island['Island']['id']]['Hose'][$hose['id']]['price']),'type'=>'hidden','readonly'=>'readonly','before'=>'<span class=\'currency\'>C$</span>'])."</td>";               
                      $shiftTableRow.="</tr>";      
                    }
