...
<?php 
  echo "<h2>".__('Island')." ".$island['Island']['name']."</h2>";
  echo "<div class='container-fluid'>";
    echo "<div class='rows'>";
      echo "<div class='col-md-6'>";
        echo "<dl>";
          echo "<dt>".__('Description')."</dt>";
          if (!empty($island['Island']['description'])){
            echo "<dd>".h($island['Island']['description'])."</dd>";
          }
          else {
            echo "<dd>-</dd>";
          }
          echo "<dt>".__('Activo')."</dt>";
          echo "<dd>".($island['Island']['bool_active']?"Activo":"Deshabilitado")."</dd>";
          echo "<dt>".__('Enterprise')."</dt>";
          echo "<dd>".($userRole == ROLE_ADMIN?($this->Html->link($island['Enterprise']['company_name'],['controller'=>'enterprises','action'=>'ver',$island['Enterprise']['id']])):$island['Enterprise']['company_name'])."</dd>";
        echo "</dl>";
...
<?php
	...
	echo "<ul>";
		if ($bool_edit_permission){ 
			echo "<li>".$this->Html->link(__('Edit Island'),['action' => 'editar', $island['Island']['id']])."</li>";
			echo "<br/>";
		} 
		if ($bool_delete_permission){ 
			//echo "<li>".$this->Form->postLink(__('Delete Island'), ['action' => 'eliminar', $island['Island']['id']], [], __('Está seguro que quiere eliminar %s?', $island['Island']['name']))."</li>";
			//echo "<br/>";
		} 
    ...
    */
		foreach ($otherIslands as $otherIsland){
			echo "<li>".$this->Html->link($otherIsland['Island']['name'], array('controller' => 'islands', 'action' => 'view',$otherIsland['Island']['id']))."</li>";
		}
	echo "</ul>";
?>
</div>
