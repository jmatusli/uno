...
<?php 
	echo "<h2>".__('Accounting Movements')."</h2>";
...
<?php
	$pageHeader="<thead>";
		$pageHeader.="<tr>";
			...
			$pageHeader.="<th>".$this->Paginator->sort('bool_debit')."</th>";
	...
	$excelHeader="<thead>";
		$excelHeader.="<tr>";
			...
			$excelHeader.="<th>".$this->Paginator->sort('bool_debit')."</th>";
		$excelHeader.="</tr>";
	$excelHeader.="</thead>";
	...
	foreach ($accountingMovements as $accountingMovement){ 
		$pageRow=""		$pageRow=""		$pageRow.="<td>".$this->Html->link($accountingMovement['AccountingRegister']['id'], array('controller' => 'accounting_registers', 'action' => 'view', $accountingMovement['AccountingRegister']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($accountingMovement['AccountingCode']['fullname'], array('controller' => 'accounting_codes', 'action' => 'view', $accountingMovement['AccountingCode']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($accountingMovement['AccountingMovement']['concept'])."</td>";
		$pageRow=""		$pageRow.="<td>".h($accountingMovement['AccountingMovement']['amount'])."</td>";
		$pageRow=""		$pageRow.="<td>".$this->Html->link($accountingMovement['Currency']['abbreviation'], array('controller' => 'currencies', 'action' => 'view', $accountingMovement['Currency']['id']))."</td>";
		$pageRow=""		$pageRow.="<td>".h($accountingMovement['AccountingMovement']['bool_debit'])."</td>";
		$pageRow=""		$pageRow=""
			$excelBody.="<tr>".$pageRow."</tr>";

			$pageRow.="<td class='actions'>";
				$pageRow.=$this->Html->link(__('View'), array('action' => 'view', $accountingMovement['AccountingMovement']['id']));
				$pageRow.=$this->Html->link(__('Edit'), array('action' => 'edit', $accountingMovement['AccountingMovement']['id']));
				//$pageRow.=->postLink(__('Delete'), array('action' => 'delete', $accountingMovement['AccountingMovement']['id']), array(), __('Are you sure you want to delete # %s?', $accountingMovement['AccountingMovement']['id']));
			$pageRow.="</td>";
